...
let activeMenu = null; // 현재 활성화된 메뉴를 추적할 변수

top_menu.forEach(function(btn) {
    btn.addEventListener('click', function() {
        // 이전 활성화된 메뉴가 있으면 비활성화
        if (activeMenu && activeMenu !== this) {
            activeMenu.querySelector(".top_menu_child").classList.remove('active');
        }

        // 현재 클릭한 메뉴의 자식 요소에 active 클래스 토글
        const child = this.querySelector(".top_menu_child");
        child.classList.toggle('active');

        // 현재 활성화된 메뉴 업데이트
        activeMenu = child.classList.contains('active') ? this : null;
    });
}); // end: forEach()
